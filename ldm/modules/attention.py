...
class BasicTransformerBlock2(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=True):
        ...
        self.attn3 = CrossAttention(query_dim=query_dim, key_dim=key_dim, value_dim=value_dim, heads=n_heads,
                                    dim_head=d_head)
        self.attn4 = CrossAttention(query_dim=query_dim, key_dim=key_dim, value_dim=value_dim, heads=n_heads,
                                    dim_head=d_head)
        self.attn5 = CrossAttention(query_dim=query_dim, key_dim=key_dim, value_dim=value_dim, heads=n_heads,
                                    dim_head=d_head)
        self.use_checkpoint = use_checkpoint
    ...
    def forward(self, x, context, objs, guidance_masks, context_subject, context_object, context_action):
        if self.use_checkpoint and x.requires_grad:
            return checkpoint.checkpoint(self._forward, x, context, objs, guidance_masks, context_subject, context_object, context_action)
        else:
            return self._forward(x, context, objs, guidance_masks, context_subject, context_object, context_action)

    def _forward(self, x, context, objs, guidance_masks, context_subject, context_object, context_action):
        _, HW, _ = x.shape
        if(HW == 64):
            all_subject_masks = guidance_masks["subject_masks_8"]  # (batch, instance_num, H, W)
            all_object_masks = guidance_masks["object_masks_8"]
            all_action_masks = guidance_masks["action_masks_8"]
        else:
            all_subject_masks = guidance_masks["subject_masks"]  # (batch, instance_num, H, W)
            all_object_masks = guidance_masks["object_masks"]
            all_action_masks = guidance_masks["action_masks"]

        x = self.attn1(self.norm1(x)) + x
        x = self.fuser(x, objs)
        x = self.attn2(self.norm2(x), context, context) + x #[b,h*w,c]

        x_expanded = x.unsqueeze(1)
        #print(x_expanded.size(), all_action_masks.size()) #torch.Size([1, 1, 4096, 320]) torch.Size([1, 60, 4096, 1280])
        masked_action_x = x_expanded * all_action_masks #[b,n,h*w,c]
        masked_action_x = masked_action_x.sum(dim=1)
        masked_object_x = x_expanded * all_object_masks
        masked_object_x = masked_object_x.sum(dim=1)
        masked_subject_x = x_expanded * all_subject_masks
        masked_subject_x = masked_subject_x.sum(dim=1)
        masked_action_x = self.attn4(masked_action_x, context_action, context_action) 
        masked_subject_x = self.attn3(masked_subject_x, context_subject, context_subject) 
        masked_object_x = self.attn3(masked_object_x, context_object, context_object) 

        masked_x = masked_action_x + masked_subject_x + masked_object_x
        x = self.attn5(self.norm4(masked_x + x), context, context) + x
        x = self.ff(self.norm3(x)) + x

        return x
    
class HOISpatialTransformer2(nn.Module):
    def __init__(self, in_channels, key_dim, value_dim, n_heads, d_head, depth=1, fuser_type=None, use_checkpoint=True):
        super().__init__()
        self.in_channels = in_channels
        query_dim = n_heads * d_head
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock2(query_dim, key_dim, value_dim, n_heads, d_head, fuser_type,
                                   use_checkpoint=use_checkpoint)
             for d in range(depth)]
        )
    ...
    def forward(self, x, context, objs, guidance_masks, context_subject, context_object, context_action):
        ...
        x = rearrange(x, 'b c h w -> b (h w) c')  # .contiguous()
        for block in self.transformer_blocks:
            x = block(x, context, objs, guidance_masks, context_subject, context_object, context_action)
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w).contiguous()
        x = self.proj_out(x)
        return x + x_in

class HOISpatialTransformer3(nn.Module):
    def __init__(self, in_channels, key_dim, value_dim, n_heads, d_head, depth=1, fuser_type=None, use_checkpoint=True):
        super().__init__()
        self.in_channels = in_channels
        query_dim = n_heads * d_head
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock3(query_dim, key_dim, value_dim, n_heads, d_head, fuser_type,
                                   use_checkpoint=use_checkpoint)
             for d in range(depth)]
        )
...
class BasicTransformerBlock3(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=True):
        ...
        self.fuser2 = GatedCrossAttentionDense(query_dim, key_dim, value_dim, n_heads, d_head)
    ...
    def _forward(self, x, context, objs):
        ...
        x = self.fuser2(x, objs)  #cross-attention后加入门控交叉注意力增强语义信息
