...
from ldm.modules.attention import HOISpatialTransformer2
from ldm.modules.attention import HOISpatialTransformer3
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context, objs, guidance_masks, context_subject, context_object, context_action):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, HOISpatialTransformer):
                x = layer(x, context, objs)
            elif isinstance(layer, HOISpatialTransformer2):
                x = layer(x, context, objs, guidance_masks, context_subject, context_object, context_action)
            # elif isinstance(layer, HOISpatialTransformer3):
            #     x = layer(x, context, objs, guidance_masks, context_subject, context_object, context_action)
            elif isinstance(layer, HOISpatialTransformer3):
                x = layer(x, context, objs)
            else:
...
class HOIUNetModel(nn.Module):
    def __init__(
        ...
        #4096*2 -> 1024*2 -> 256*2 -> 64 -> 256*3 -> 1024*3 -> 4096*3
        # = = = = = = = = = = = = = = = = = = = = Down Branch = = = = = = = = = = = = = = = = = = = = #
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    # if level == 2 :
                    #     layers.append(HOISpatialTransformer2(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads,
                    #                                     d_head=dim_head, depth=transformer_depth, fuser_type=fuser_type,
                    #                                     use_checkpoint=use_checkpoint))
                    # else:
                    #     layers.append(HOISpatialTransformer3(ch, key_dim=context_dim, value_dim=context_dim, n_heads=num_heads,
                    #                                     d_head=dim_head, depth=transformer_depth, fuser_type=fuser_type,
                    #                                     use_checkpoint=use_checkpoint))
    ...
    def forward(self, input):
        ...
        if self.training and random.random() < 0.1 and self.grounding_tokenizer_input.set:  # random drop for guidance
            grounding_input = self.grounding_tokenizer_input.get_null_input()
        object_masks = input["object_masks"]
        object_masks = object_masks.view(object_masks.size(0), object_masks.size(1), -1)
        object_masks = object_masks.unsqueeze(3).expand(-1, -1, -1, 1280) #[b,90,256,1280]
        subject_masks = input.get("subject_masks", None)
        subject_masks = subject_masks.view(subject_masks.size(0), subject_masks.size(1), -1)
        subject_masks = subject_masks.unsqueeze(3).expand(-1, -1, -1, 1280) #[b,90,256,1280]
        action_masks = input.get("action_masks", None)
        action_masks = action_masks.view(action_masks.size(0), action_masks.size(1), -1)
        action_masks = action_masks.unsqueeze(3).expand(-1, -1, -1, 1280) #[b,90,256,1280]

        object_masks_8 = input["object_masks_8"]
        object_masks_8 = object_masks_8.view(object_masks_8.size(0), object_masks_8.size(1), -1)
        object_masks_8 = object_masks_8.unsqueeze(3).expand(-1, -1, -1, 1280) #[b,90,256,1280]

        subject_masks_8 = input.get("subject_masks_8", None)
        subject_masks_8 = subject_masks_8.view(subject_masks_8.size(0), subject_masks_8.size(1), -1)
        subject_masks_8 = subject_masks_8.unsqueeze(3).expand(-1, -1, -1, 1280) #[b,90,256,1280]

        action_masks_8 = input.get("action_masks_8", None)
        action_masks_8 = action_masks_8.view(action_masks_8.size(0), action_masks_8.size(1), -1)
        action_masks_8 = action_masks_8.unsqueeze(3).expand(-1, -1, -1, 1280) #[b,90,256,1280]
        #print(action_masks.shape) [2, 1, 16, 16])
        ...
        context_subject = input["context_subject"].to(dtype=objs.dtype)
        context_object = input["context_object"].to(dtype=objs.dtype)
        context_action = input["context_action"].to(dtype=objs.dtype)

        # Start forwarding
        hs = []
        for module in self.input_blocks:
            h = module(h, emb, context, objs, guidance_masks, context_subject, context_object, context_action)
            hs.append(h)

        h = self.middle_block(h, emb, context, objs, guidance_masks, context_subject, context_object, context_action)
